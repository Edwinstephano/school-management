...
export default function MeetingsPage() {
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-50">Parent Meetings</h1>
        <p className="text-sm text-slate-600 dark:text-slate-400">Schedule and view parent meetings</p>
      </header>
      <div className="grid gap-6 md:grid-cols-[minmax(0,1.3fr)_minmax(0,1.7fr)]">
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="mb-3 text-xs uppercase tracking-wide text-slate-600 dark:text-slate-400">New Meeting</div>
          <form onSubmit={handleSubmit} className="space-y-3 text-sm">
            <div>
              <label className="mb-1 block text-xs text-slate-700 dark:text-slate-300">Title</label>
              <input
            ...
            <div>
              <label className="mb-1 block text-xs text-slate-700 dark:text-slate-300">Description</label>
              <textarea
                className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-slate-900 outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
                rows={3}
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <div>
                <label className="mb-1 block text-xs text-slate-700 dark:text-slate-300">Time</label>
                <input
                  type="time"
                  className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-slate-900 outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
                  value={form.time}
            ...
            <div>
              <label className="mb-1 block text-xs text-slate-700 dark:text-slate-300">Location</label>
              <input
                className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-slate-900 outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
                value={form.location}
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="mb-3 text-xs uppercase tracking-wide text-slate-600 dark:text-slate-400">Upcoming Meetings</div>
          <div className="space-y-2 text-sm text-slate-700 dark:text-slate-100">
            {meetings.map((m) => (
              ...
              >
                ...
                {m.description && <div className="mt-2 text-xs text-slate-600 dark:text-slate-300">{m.description}</div>}
              </div>
            ))}
            {meetings.length === 0 && <div className="text-sm text-slate-500 dark:text-slate-500">No meetings scheduled.</div>}
          </div>
        </div>
      </div>
    </div>
