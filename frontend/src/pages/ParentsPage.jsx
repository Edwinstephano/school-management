...
export default function ParentsPage() {
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-2xl font-semibold text-slate-50">Parents</h1>
        <p className="text-sm text-slate-400">Create parent login accounts and profiles</p>
      </header>
      <div className="grid gap-6 md:grid-cols-[minmax(0,1.7fr)_minmax(0,1.3fr)]">
        <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-4">
          <div className="mb-3 text-xs uppercase tracking-wide text-slate-400">All Parents</div>
          <div className="space-y-2 text-sm text-slate-100">
            {parents.map((p) => (
              <div key={p.id} className="rounded-lg border border-slate-800 bg-slate-900/70 p-3">
                ...
                <div className="mt-1 text-xs text-slate-400">{p.phone}</div>
                {p.address && <div className="mt-1 text-xs text-slate-500">{p.address}</div>}
              </div>
            ))}
            {parents.length === 0 && <div className="text-sm text-slate-500">No parents yet.</div>}
          </div>
        </div>
        <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-4">
          <div className="mb-3 text-xs uppercase tracking-wide text-slate-400">Add Parent</div>
